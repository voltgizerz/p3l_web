...
class Cs extends CI_Controller
{
    ...
    public function transaksi_penjualan_produk()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            //PAGINATION SETTING
            $config = array();
            $config["base_url"] = base_url() . "cs/transaksi_penjualan_produk";
            ...
            $data['dataPenjualanProduk'] = $this->menu->getDataPenjualanProdukAdmin($config["per_page"], $page);
    ...
    public function transaksi_penjualan_layanan()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->load->view('templates/topbar', $data);
              ...
              //PAGINATION SETTING
              $config = array();
              $config["base_url"] = base_url() . "cs/transaksi_penjualan_layanan";
              $config["total_rows"] = $this->menu->get_count();
              $config["per_page"] = 10;
              $config["uri_segment"] = 3;
              ...
              $page = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
              $data["links"] = $this->pagination->create_links();
              $data['dataPenjualanLayanan'] = $this->menu->getDataPenjualanLayananAdmin($config["per_page"], $page);
